const addCards = (items) => { 
    ...
    items.forEach(item => {
        let itemToAppend = 
            ...
            '<span class="card-title activator grey-text text-darken-4">'+item.title+'<i class="material-icons right">more_vert</i></span><p><a href="#">'+item.color+'</a></p></div>'+ 
...
const postcards = (formData) => {
    ...
    $.post('/api/cats', formData, (response) => {
        if (response.statusCode == 201) {
            ...
            getcards();
            $('#modal1').modal('close'); // Changed to 'close'
        } else {
